...
export class OrderController {
  ...
  checkout = async (request: Request, response: Response, next: NextFunction) => { try { response.status(httpStatus.created).json({ order: await this.service.checkout(this.user(request)) }); } catch (error) { next(error); } };
